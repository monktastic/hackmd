...
The founders of QM found its implications _unfathomably_ strange. There is a growing modern belief that they simply misunderstood, and that modern findings have resolved the central weirdness.

This is a misconception. Modern interpretations have not resolved the weirdness, but have only obscured it behind technical details. I hope to un-obscure some of those details so that you can stare that weirdness in the face yourself.

---

A single particle can be in a **superposition**. This means that it is not actually in one state or the other, and crucially, this can be _proven_ by something called an **interference experiment.** As other particles interact with the superposition, they simply _join_ it (i.e., become **entangled** with it). In this way, the superposition just keeps growing and growing forever[^collapse].

[^collapse]: Interpretations of QM can be broadly divided into _collapse_ and _no-collapse_. The former state that superpositions undergo a real physical collapse. Technically, their distinct predictions make them distinct _theories_, and not _interpretations_ of QM. The vast majority of physicists are in the no-collapse camp. This piece takes the no-collapse perspective, and is agnostic about interpretation.

Doing an interference experiment requires careful control over **all** particles in the superposition. If you lose track of any of them, or conduct an experiment on only a _subset_, you will no longer see interference. This has a few immediate consequences:

1. In the double-slit experiment, once the electron entangles with the detector at the slits (which is what makes it a "detector" in the first place), the electron _by itself_ can no longer show interference. This is a well-studied effect of entanglement. There is nothing spooky going on here, and it has nothing to do with consciousness, despite claims to the contrary:
...
(**Update**: it is also not because of the detector adding a random phase to one or both paths, as this [PBS Spacetime episode](https://www.youtube.com/watch?v=GlOwJWJWPUs) wrongly explains. If that were true, we would expect to see a smeared-out blur instead of the two bright classical-looking bands that we actually see. If you enjoy math, you can see the details worked out [here](https://physics.stackexchange.com/a/205121).)

Sometimes people think of such a measurement as "collapse of the wavefunction," but this is not quite correct. The electron + detector system is still in a _joint_ superposition (of _left_ and _right_ outcomes), which then grows to include the screen, etc.

2. When information is dissipated into the environment (such as air molecules and photons), performing an interference experiment quickly becomes hopeless. Any experiment you try to run will show only classical results.
...
So why do Carroll et al. use words like "impossible" and "forever" when they know perfectly well that they are not literally true? A hint can be found in Carroll's next claim:

>  It is as if they have become distinct worlds — because they have.
...
So far we have said that quantum events attain definite results when they reach _you._ But as far as we know, _which_ result is obtained is completely random. This means that:
